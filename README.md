...
> 1️⃣ **Step 1:** Build and test the real product.  
> 2️⃣ **Step 2:** Upload real screenshots and create a working demo.  
> 3️⃣ **Step 3:** Create the Gumroad product, collect its immutable product ID, and configure the custom checkout field below.  
> 4️⃣ **Step 4:** Upload the release file to the private `releases/…` Firebase Storage path—or configure a protected app-access URL.  
> 5️⃣ **Step 5:** In `/admin`, create the product as a **draft**. Fill every required field and private delivery setting.  
> 6️⃣ **Step 6:** Check the preview and product page. Only then publish it.  
...
>  **1. Access Token**  
> Create a Gumroad API access token with sales access. Keep it secret.

>  **2. Custom Field**  
> For **each Gumroad product sold through ARIX**, create an optional text custom field named exactly:  
> ```text
> arix_checkout_token
> ```  
...
>  **3. Firebase Secret**  
> Set the Firebase secret after installing the Functions dependencies:  
> ```powershell
> cd functions
> npm install
...
> Deploy Functions, then use the resulting `gumroadWebhook` URL as the Gumroad webhook endpoint. For this Firebase project it will normally be:  
> ```text
> [https://us-central1-arix-website.cloudfunctions.net/gumroadWebhook](https://us-central1-arix-website.cloudfunctions.net/gumroadWebhook)
> ```

> 📡 **5. Event Subscription**  
> In Gumroad, subscribe this endpoint to at least `sale`, `refund`, and `dispute` events (also use `cancellation` and `subscription_ended` for subscriptions). Gumroad’s resource subscription API can register them; Gumroad Ping may be used for a sale notification if that is what your account exposes.
> 
>  **VERIFICATION LOGIC**  
...
> 1️⃣ Sign in once with the chosen Google account.  
> 2️⃣ Copy its Firebase Auth UID.  
> 3️⃣ In Firestore, create `admins/<UID>` with `{ role: 'admin' }`.  
>   
